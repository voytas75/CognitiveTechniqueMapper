...
def test_create_catalog_service_uses_explicit_dependencies(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Catalog construction does not need CLI-global monkeypatch overrides."""
    ...
    catalog, returned_sqlite = cli_runtime.create_catalog_service(
        config_service_cls=StubConfigService,
        sqlite_client_cls=sqlite_client_factory,
        embedding_gateway_cls=embedding_gateway_factory,
        technique_catalog_service_cls=StubCatalog,
        chroma_client_cls=None,
    )
