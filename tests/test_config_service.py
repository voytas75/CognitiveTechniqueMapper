...
def test_config_service_loads(tmp_path: Path, monkeypatch) -> None:
    ...
    (config_dir / "settings.yaml").write_text("app: {name: test, version: '0.0.1'}\n", encoding="utf-8")
    (config_dir / "database.yaml").write_text("database: {sqlite_path: ':memory:'}\n", encoding="utf-8")
    (config_dir / "models.yaml").write_text(
        "workflows: {detect_technique: {model: dummy}}\ndefaults: {provider: mock}\n", encoding="utf-8"
    )
    (config_dir / "providers.yaml").write_text("providers: {mock: {api_base: 'http://localhost'}}\n", encoding="utf-8")
